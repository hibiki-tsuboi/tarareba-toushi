...
- `accessibilityIdentifier` はUIテストとの契約です（`simulate` / `refresh-data` / `data-info` / `edit-input` / `input-error` / `comparison-difference` / `investment-plan` / `principal-summary` / `chart-principal` / `lump-sum-summary` など）。評価額と損益、グラフの読み取り行、一括との比較の行は `valuation-<商品ID>` / `profit-<商品ID>` / `chart-value-<商品ID>` / `lump-sum-<商品ID>` と商品IDから組み立てるため、商品IDを変えるとUIテストの参照先も変わります。
