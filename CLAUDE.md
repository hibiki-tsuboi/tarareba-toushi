...
「たられば投資」＝オルカンとS&P500に同額を一括投資していた場合を比較するiOSアプリと、その価格データを配信するCloudflare Workersの2つで構成されたリポジトリです。ドキュメント・コミットメッセージ・UI文言・エラーメッセージはすべて日本語で書きます。
...
- 保存済み履歴がなければ**通信前に停止**します。初回全履歴の構築だけ `npm run fetch:mufg -- --backfill` を明示。
