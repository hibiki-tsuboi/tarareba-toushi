...
`fetch:mufg` は投信情報APIだけを使用し、日付と通常の基準価額を保存します。CSVの取得・解析は行いません。2商品の最新値を確認し、保存済み最終日の翌日から不足分を日付指定APIで順に補います。既存の過去日は再取得しません。変化がなければ最新値APIの2回だけで終わります。商品識別・日付・正数・APIのエラーを検証し、直列リクエストの間に1秒待ちます。アプリからUFJへの直接通信は行いません。

通常基準価額への移行は実施済みです。`npm run migrate:nav` は確認済みの既存履歴を通信なしで移行するコマンドで、任意の再投資系列を変換する機能ではありません。過去の価格を再取得せず、日付・値をそのまま保持しています。

履歴がない場合は通常の取得コマンドが通信前に停止します。将来、初回の全履歴構築が必要な場合だけ `npm run fetch:mufg -- --backfill` を使用します。設定日から1日ずつAPIへ問い合わせるため時間がかかります。既存履歴があれば、この引数を付けても差分取得です。今回の変更作業では取得コマンドを実行していません。
