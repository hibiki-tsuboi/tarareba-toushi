...
export function validate(snapshot) {
    ...
    assert.deepEqual(m.funds.map(f => f.id).sort(), [...ids].sort());
    assert.equal(snapshot.series.length, 2);
    assert.deepEqual(snapshot.series.map(s => s.fundId).sort(), [...ids].sort());
    let common;
    for (const f of m.funds) {
        assert.equal(f.currency, 'JPY');
        assert(f.displayName.includes('サンプル') && f.displayName.length <= 80);
        ...
        assert.equal(s.source.kind, 'synthetic');
