...
選んだ投資信託へそれぞれ同じ金額を一括投資していた場合の結果を比較するSwiftUIアプリです。**eMAXIS Slim 全世界株式（オール・カントリー）・米国株式（S&P500）・国内株式（TOPIX）・国内株式（日経平均）と eMAXIS NASDAQ100インデックスの公式データ**を使用します。広告・課金・ログインはありません。
