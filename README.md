...
投資信託を選び、過去に一括投資していた場合の金額を確認するSwiftUIアプリです。**eMAXIS Slim 全世界株式（オール・カントリー）と米国株式（S&P500）の公式データ**を使用します。広告・課金・ログインはありません。
...
- 最初にオルカン／S&P500の2択から投資信託を選び、開始日と投資金額を入力します。「シミュレート」を押すと別の結果画面へ進み、選んだ商品の増減額と投資後の金額だけを表示します。「条件を変更する」または戻る操作で、選択・入力値を保ったまま入力画面へ戻れます。
- 投資信託の初期選択はオルカンです。シミュレーションに成功したときに商品・開始日・金額をまとめて保存し、再起動後も復元します。商品選択を含まない以前の保存データでは、開始日・金額を保ち、オルカンを選択します。
